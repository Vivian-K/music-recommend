...
def readData():
    ###采用fast_FM数据
    '''
    data格式：[[user映射整数， 艺术家， plays归一化], [], []]
    '''
    dataHome = './testdata/'
    small_data = pd.read_csv(filepath_or_buffer=dataHome + 'small_data.csv',encoding="UTF-8")
    small_data['user'] = small_data['user'].astype("category")#转换为分类类型数据
    small_data['artist'] = small_data['artist'].astype("category")
    user_artist = list(zip(list(small_data['user'].cat.codes),list(small_data['artist'].cat.codes)))#cat.codes来实现对整数的映射
...
#        result =  ItemCF.Recommendation(test.keys(),train, W)
    ...
    #    rank = ItemCF_IUF.Recommend('1',train,W)
        ...
        #F=15:15个隐类；T=40：迭代40次；alpha=0.02：学习率；lamb=0.01：正则化参数
